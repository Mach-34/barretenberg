...
import { fetchCode, getNumCpu, createWorker, getRemoteHalo2Wasm, threadLogger, killSelf } from './node/index.js';
...
export class Halo2Wasm {
    ...
    private remoteWasms: Halo2WasmWorker[] = [];
    ...
    public static async new() {
        const halo2 = new Halo2Wasm();
        await halo2.init(1);
        return halo2;
    }

    /**
     * Construct and initialise Halo2Wasm within a Worker. Return both the worker and the wasm proxy.
     * Used when running in the browser, because we can't block the main thread.
     */
    public static async newWorker(threads?: number) {
        const worker = createWorker();
        const wasm = getRemoteHalo2Wasm(worker);
     ...
     */
    public async init(
        threads = Math.min(getNumCpu(), Halo2Wasm.MAX_THREADS),
        ...
        // const code = await fetchCode(threads > 1 ? 'halo2-threads.wasm' : 'halo2.wasm');
        // single threded wasm for now, axiom can provide multithreaded halo2 wasm
        const code = await fetchCode('halo2.wasm');
        ...
        this.remoteWasms = await Promise.all(this.workers.map(getRemoteHalo2Wasm));
    ...
    private getImportObj(memory: WebAssembly.Memory) {
        ...
        const importObj = {
            ...
            // We literally only need to support random_get, everything else is noop implementated in halo2.wasm.
...
export type Halo2WasmWorker = Remote<Halo2Wasm>;
